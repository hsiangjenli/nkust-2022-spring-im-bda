def filter_df_via_content(query_keywords, cond, cate, weeks):
    ...
    # start date
    start_date_delta = (end_date.date() - timedelta(weeks=weeks))
...
def get_daily_basis_sentiment_count(df_query, sentiment_type='pos', freq_type='D'):
    ...
    if sentiment_type == 'pos':
        ...
        lambda_function = lambda senti: 1 if senti <= 0.3 else 0 #小於0.4為負向
    elif sentiment_type == 'neutral':
        lambda_function = lambda senti: 1 if senti > 0.3 & senti < 0.3 else 0 #介於中間為中立
    else:
        return None 
